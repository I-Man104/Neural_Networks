...
def plotting(weights,bias,X_train,X_test,y_train,y_test):
  plt.scatter(X_test[y_test == 0][:, -1], X_test[y_test == 0][:, 1], label=f'Class BOMBAY')
  plt.scatter(X_test[y_test == 1][:, -1], X_test[y_test == 1][:, 1], label=f'Class SIRA')
  w0, w1 = weights
#   bias = bias_value
...
weights,bias_value,X_train,X_test,y_train,y_test = single_preceptron(0.1, 100, 0.01, ['Area','Perimeter'], ['BOMBAY','SIRA'], bias=False)
